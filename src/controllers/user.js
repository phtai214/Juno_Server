...
export const creatUserForAdmin = async (req, res) => {
  ...
  try {
    const result = await services.createNewUserForAdmin({
      name,
      email,
      password,
      phonenumber,
...
export const creatUserEmployee = async (req, res) => {
  const { name, email, password, position, status, role, permissions } =
    req.body; // Thêm position và status
  try {
    const result = await services.createNewUserEmployee({
      name,
      email,
      password,
      position,
...
export const deleteUser = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: `Error deleting user by ID: ${error.message}` });
  }
};
export const updatePermissions = async (req, res) => {
  try {
    ...
    const updatedUser = await services.updateEmployeePermissions(
      id,
      permissions
    );
    res.status(200).json({
    ...
    res
      .status(500)
      .json({ message: "Lỗi khi cập nhật quyền hạn", error: error.message });
  }
};
...
export const fetchPermissions = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Lỗi khi lấy quyền hạn nhân viên",
      error: error.message,
    });
  }
};
